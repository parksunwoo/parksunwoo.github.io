...
[어느날 갑자기 장애 - Istio Upgrade](https://parksunwoo.github.io/dev/2023/05/20/a-istio-failure.html)

해당 내용의 글이 게시되고 얼마 지나지 않아 한통의 메일을 받게되었다.

메일을 처음 확인했을때 보내주신 분(이하 A님) 이 석연치 않았던 부분을 정확히 지적해줘서 

내가 느끼는 부끄러움은 아주 잠시였고 고마운 마음이 더 컸다.
...
공교롭게도, 쿠버네티스 v1.22부터 Ingress에 해당하는 `networking.k8s.io` API와 ValidatingWebhookConfiguration에 해당하는 `admissionregistration.k8s.io` API는 Beta 버전인 v1beta1은 deprecate되고 GA버전인 v1으로 변경되었습니다. ([관련 GKE 문서](https://disq.us/url?url=https%3A%2F%2Fkubernetes.io%2Fblog%2F2021%2F07%2F14%2Fupcoming-changes-in-kubernetes-1-22%2F%3AP4ESaLW1QhvJIXIIqC_p4SBZE0k&cuid=5888907), [관련 쿠버네티스 문서](https://disq.us/url?url=https%3A%2F%2Fkubernetes.io%2Fblog%2F2021%2F07%2F14%2Fupcoming-changes-in-kubernetes-1-22%2F%3AP4ESaLW1QhvJIXIIqC_p4SBZE0k&cuid=5888907))

또한, 운영환경에서 사용하시던 istio 1.7.4는 소스를 분석해보니 deprecate된 `networking.k8s.io/v1beta1`, `admissionregistration.k8s.io/v1beta1` 을 사용하고 있습니다.
...
꽤 시일이 지난 과거이지만 운영환경의 GKE 버전을 확인할수 있는 방법을 찾아야했다.([링크](https://cloud.google.com/kubernetes-engine/docs/concepts/cluster-upgrades?hl=ko#check-update-log))
...
GCP 상에 로그탐색기에서 위 쿼리를 실행하니 클러스터 업그레이드 내역을 조회할수 있었다. 

내역을 조회해 보니 A님이 이야기한 부분이 충분히 가능한 이야기라 생각되었고 istio 지원종료가 원인이 아닐수 있겠다는 생각이 들었다

이슈가 발생한 23년 5월 18일 20시 이전에 gke 업그레이드 로그가 있는지 확인하였을때 
...
(3) istiod 에러 로그에서 *v1beta1.Ingress , *v1beta1.ValidatingWebhookConfiguration 리소스를 찾을 수 없다는 에러발생
...
istio의 1.7 버전이 오래된 버전이라 commit 기록을 통해서 해당 정보를 찾을 수 있었다.

![Istio Commit](/assets/images/istio-commit-1.png)

[Github 링크](https://github.com/istio/istio.io/commit/778b410748c985b6f7949044ebd51c519109efbb)
...
[Github 링크](https://github.com/istio/istio.io/commit/f0658ddc6899cc6ab7048b6f1825552f7693de48)

위 내용들을 정리해 A 님께 답장 메일을 보냈다.
...
> istio 1.13.3 소스 코드를 확인하면, ValidatingWebhookConfiguration 및 Ingress 관련 코드에서 v1 API 사용 중
...
저도 여기서 막혔는데, 한 가지 실마리로 다음을 찾았습니다. [Google Cloud 링크](https://cloud.google.com/kubernetes-engine/docs/deprecations/apis-1-23?hl=ko)
...
GKE의 패치버전 1.22.17-gke.**5400** -> 1.22.17-gke.**7500** 은 별도의 업그레이드 정보가 없는지가 아직 풀리지 않은 부분입니다.
...
여기서 webhoook에 대해 잠시 정리해보고 가면

MutatingWebhookConfiguration 과 ValidatingWebhookConfiguration 은 무엇이고 어떤 역할은 할까?

- **MutatingWebhookConfiguration:** Pod 스펙을 변경하는 Webhook을 정의
    - **사용 사례:**
...
1.23 버전부터 MutatingWebhookConfiguration과 ValidatingWebhookConfiguration API v1beta1이 지원 중단

**변경 사항 요약:**

- **v1beta1 API 더 이상 사용 불가능:** 1.23 버전 이상의 GKE 클러스터는 v1beta1 API를 지원하지 않는다.
- **v1 API 사용 필요:** Webhook을 계속 사용하려면 v1 API로 마이그레이션 필요
- **마이그레이션 과정:**
...
그럼 여기서 다시 한가지 질문을 더 해보면 k8s는 어느 주기로 업그레이드를 하고 또 이에 대응하기 위해서는 어떻게 해야할지 의문이 생길 것이다.

**Kubernetes 버전 업그레이드 주기는**
- **마이너 버전:** 3개월마다 출시. 버그 수정 및 안정성 향상에 중점
- **메이저 버전:** 1년마다 출시. 새로운 기능 추가 및 기존 기능 개선에 중점

이에 따라 권장하는 **업그레이드 주기는**
- **프로덕션 환경:** 최소 6개월 이상 버전을 유지하고, 지원 종료 3개월 전에 업그레이드
- **테스트 환경:** 최신 버전을 유지하여 새 기능 테스트 및 업그레이드 프로세스 검증

업그레이드 전에 항상 백업을 먼저 수행해야하고 패키지 버전 관리를 위한 최적의 방법으로는 3가지가 보통 이야기된다.
- **버전 호환성 매트릭스 참조:** Kubernetes, Istio 및 기타 사용 중인 패키지의 버전 호환성 매트릭스를 Github 또는 Docs를 통해 확인한다.
- **자동화 도구 활용:** ArgoCD와 같은 자동화 도구를 사용하여 배포 프로세스 자동화 및 버전 관리한다.
- **테스트 환경에서 업그레이드 테스트:** 프로덕션 환경에 적용하기 전에 테스트 환경에서 업그레이드 테스트를 먼저 수행해보고 이후에 적용한다.

---
.
.
.
이번 경험에서 지금까지와는 다른게 느낀 부분이 있다면

작성한 블로그 글을 통해 확실히 정리하지 못한 부분을 다시금 짚고 넘어갈 수 있어서 좋았고

또한 작성한 글을 통해 누군가와 이야기하고 문제의 원인을 찾아갈 수 있어서 기분 좋은 경험이었다.

기분 좋은 경험을 제공해주신 A님, 이자리를 빌어 감사의 인사드립니다 :)