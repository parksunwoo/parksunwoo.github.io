...
DAO (Data Access Object) 는 데이터베이스의 data에 접근하기 위한 객체. 데이터베이스에 접근 하기 위한 로직 & 비즈니스 로직을 분리하기 위한 목적
...
DTO (Data Transfer Object) 는 계층 간 데이터 교환을 하기 위해 사용하는 객체, DTO는 로직을 가지지 않는 순수한 데이터 객체(getter, setter만 가진 클래스) 
...
VO (Value Object) 는 값 오브젝트로써 값을 위해 사용. read-only 특징( 사용하는 도중에 변경 불가능하여 오직 읽기만 가능)을 가집니다
